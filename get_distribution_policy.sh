...
psql -d $PGDATABASE -p $PGPORT -Atc "SELECT 
	...
	AND relnamespace IN (
		...
		WHERE nspname NOT IN (
			'gp_toolkit','pg_toast','pg_aoseg','information_schema','pg_catalog', 'madlib')
		AND nspname NOT LIKE 'pg_temp%')
	ORDER BY 3" > $extract_table
}
...
do 
	...
	psql -d $PGDATABASE -p $PGPORT -xtc "SELECT
		localoid, attrnums
	...
	then
		...
		psql -d $PGDATABASE -p $PGPORT -Atc "SELECT attname 
			FROM pg_attribute
			WHERE attrelid IN ($table_oid) 
			AND attnum IN ($table_dist_col)" | awk  '{ printf $1 ","}' | sed s/.$// > $ext_tab_parent_dist_col_info
